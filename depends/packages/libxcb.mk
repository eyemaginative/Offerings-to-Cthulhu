...
# Static libs: promote each xcb*.pc's Requires.private to Requires so a plain
# (non-static) `pkg-config --libs xcb*` chains the dependency libs (libXau etc.).
# Qt 5's xcb config tests use non-static pkg-config and otherwise link only
# -lxcb, which can't resolve libXau's Xau* symbols against the static libxcb.a.
define $(package)_postprocess_cmds
  rm -rf share lib/*.la && \
  sed -i 's/^Requires.private:/Requires:/' lib/pkgconfig/xcb*.pc
endef
